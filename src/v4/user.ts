...
export default (client: any, handler: any): User => ({
  get: ({ clientOptions }: { clientOptions: any }): Promise<[{ code: 200, data: { id: string, key: string, created_at: string } }]> => handler.apply({ method: 'get', url: 'api/v4/user', resource: 'user', variable: [{ name: 'key_id', type: 'string' }], headers: { Accept: 'application/json' }, query: [], data: null }, [client, { clientOptions }]),
  "keys": {
